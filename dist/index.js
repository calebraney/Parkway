(() => {
  ...
  var accordion = function(gsapContext) {
    ...
    accordionLists.forEach((list) => {
      ...
      if (runOnBreakpoint === false)
        return;
      let firstOpen = attr2(false, list.getAttribute(OPTION_FIRST_OPEN));
      let oneActive = attr2(false, list.getAttribute(OPTION_ONE_ACTIVE));
      let keepOneOpen = attr2(false, list.getAttribute(OPTION_KEEP_ONE_OPEN));
      let hoverOnly = attr2(false, list.getAttribute(OPTION_HOVER_OPEN));
      ...
      if (hoverOnly) {
        const accordionItems2 = list.querySelectorAll(ACCORDION_ITEM);
  ...
  var mouseOver = function(gsapContext) {
    ...
    mouseOverItems.forEach((mouseOverItem) => {
      ...
      const mouseMove = function() {
        ...
        let duration = attr2(0.5, mouseOverItem.getAttribute(MOUSEOVER_DURATION));
        let ease = attr2("power1.out", mouseOverItem.getAttribute(MOUSEOVER_EASE));
        ...
        layers.forEach((layer) => {
          let moveX = attr2(10, layer.getAttribute(MOUSEOVER_MOVE_X));
          let moveY = attr2(10, layer.getAttribute(MOUSEOVER_MOVE_Y));
          let rotateZ = attr2(0, layer.getAttribute(MOUSEOVER_ROTATE_Z));
  ...
  var parallax = function(gsapContext) {
    ...
    parallaxItems.forEach((parallaxItem) => {
      ...
      if (!parallaxItem || !section || !trigger)
        return;
      let animationType = "uncover";
      animationType = attr2("uncover", parallaxItem.getAttribute(PARALLAX_TYPE));
      moveAmount = attr2(50, parallaxItem.getAttribute(PARALLAX_AMOUNT));
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    const focusText = function() {
      ...
      if (textLinks.length === 0 || images.length === 0)
        return;
    ...
    const checkFilterDropdowns = function() {
      ...
      dropdowns.forEach((dropdown, index) => {
        ...
        if (!filterText || checkboxes.length === 0)
          return;
        let activeCheckboxes = false;
        const checkForActiveCheckboxes = function() {
          const activeCheckboxes2 = Array.from(checkboxes).filter(function(checkbox) {
            return checkbox.checked;
          });
          if (activeCheckboxes2.length > 0) {
            filterText.classList.add(ACTIVE_CLASS);
            filterText.textContent = `(${activeCheckboxes2.length})`;
          } else {
    ...
    const portfolioSlider = function() {
      ...
      sliders.forEach((slider) => {
        ...
        const timerDuration = attr2(TIMER_DURATION, slider.getAttribute(OPTION_TIMER_DURATION));
        ...
        const activateSlides2 = function(activeIndex) {
          slides.forEach((slide2, index) => {
            slide2.classList.remove(activeClass);
            slide2.classList.remove(beforeClass);
            slide2.classList.remove(afterClass);
            if (index === activeIndex) {
              slide2.classList.add(activeClass);
            }
            if (index < activeIndex) {
              slide2.classList.add(beforeClass);
            }
        ...
        const nextSlide = function(nextIndex = void 0) {
          if (nextIndex === void 0) {
            nextIndex = findNextIndex();
          }
          activateSlides2(nextIndex);
          clearInterval(timer);
          startTimer();
        };
        nextSlide(0);
        ...
        headings.forEach((heading, index) => {
          heading.addEventListener("click", (event2) => {
            nextSlide(index);
          });
        });
    ...
    const approachTestimonialSlider = function() {
      ...
      swipers.forEach(function(swiper) {
        ...
        if (!nextButtonEl || !previousButtonEl || !swiperList)
          return;
